@extends('dashboard')
@section('content')
    <a class="btn btn-primary mb-4" href="{{ route('user.tambah') }}">Tambah User</a>
    @if (session('message'))
            ...
            <tbody>
                ...
                @foreach ($user as $u)
                    <tr>
                        ...
                        <td>{{ $u->email }}</td>
                        <td>{{ $u->rule }}</td>
                        <td><img width="100" height="100" style="object-fit: cover;border-radius:100%;" src="/foto_profile/{{ $u->foto_profile }}"/></td>
                        <td>
                        <div style="gap: 10px; height:100%" class="d-flex flex-wrap">
                            ...
                            <!-- Modal -->
                            <div class="modal fade" id="hapusUser{{ $u->id }}" tabindex="-1"
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form action="{{ route('user.delete', $u->id) }}" method="post">
                                                @csrf
                                                <div class="modal-body">
                                                    Apakah anda yakin ingin menghapus user {{ $u->username }}?
                                                    <input type="hidden">
                                                    <br>
                                                    <br>
